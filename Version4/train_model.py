...
def preprocess_data(df, time_steps=60):
    ...
    # Important: "sentiment_label" is the column name
    ...
    for i in range(len(scaled_data) - time_steps):
        ...
        # 'Close' is index=1 in numeric_features
...
def main():
    ...
    # If "Adj Close" is missing, create it by copying "Close"
    if "Adj Close" not in df.columns:
        print("No 'Adj Close' in CSV. Creating it by copying 'Close'...")
    ...
    # Because 'Close' was index=1 in our scaled data, we'll invert just that column
    y_test_actual = scaler.inverse_transform(
        np.hstack(
            (
                np.zeros((len(y_test), X_test.shape[2] - 1)),
                y_test.reshape(-1, 1)
            )
        )
    )[:, -1]
    y_pred_actual = scaler.inverse_transform(
        np.hstack(
            (
                np.zeros((len(y_pred), X_test.shape[2] - 1)),
                y_pred
            )
        )
    )[:, -1]
